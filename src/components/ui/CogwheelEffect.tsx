...
const CogwheelEffect: React.FC = () => {
  ...
  const reverseRotate = useTransform(rotate, (value) => -value); // move to top
  ...
  if (isMobile) return null; // ✅ SAFE NOW — after all hooks
  ...
  return (
    <div className="fixed left-4 top-1/2 -translate-y-1/2 z-10 hidden md:block">
      <div className="relative">
        ...
        <motion.div style={{ rotate: reverseRotate }} className="absolute top-16 left-0 text-secondary">
          <Cog size={36} className="opacity-60" />
        </motion.div>
        <motion.div style={{ rotate }} className="absolute top-32 left-4 text-accent">
